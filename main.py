...
class MyFrame(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        self.SetStatusText("Powered by bobo, 2015/08", 0)
...
#             apks = cfg.get("apk")
        ...
        btnOpenApk = wx.Button(panel, -1, "APK")
...
#         if len(apks) > 0 and apks[0] != None :
#             apk.SetLabelText(apks[0]) 
        if cfg != None and cfg.get("apk", None) != None:
            apk.SetLabelText(cfg.get("apk"))
...
#         self.apk2 = apk
        ...
        l3 = wx.Button(panel, -1, "ApkName")
        t3 = wx.TextCtrl(panel, -1, "", size=(300, -1))
        #t3.SetHelpText()
        t3.SetHint("Input build ApkName, such as qmbb-v1.0.10.apk")
        t3.SetLabelText("qmbb-v1.0.10.apk")
        wx.CallAfter(t3.SetInsertionPoint, 0)
        self.apkName = t3
        
        l4 = wx.Button(panel, -1, "aliasName")
        t4 = wx.TextCtrl(panel, -1, "", size=(300, -1))
        #t4.SetHelpText()
        t4.SetHint("Input build ApkName, such as c:\newtank\newtank\quanminbb.keystore")
        t4.SetLabelText("c:\\newtank\\newtank\quanminbb.keystore")
        wx.CallAfter(t4.SetInsertionPoint, 0)
        self.aliasName = t4
...
#         sizer.AddSizer(line8)
        ...
        sizer.Add(btnBuildScript, 0, wx.ALL, 11)
    ...
    def OnResetApks(self, evt):
        """Event handler for the button click."""
        print "OnResetApks"
        self.apk.SetLabelText("No selected")
...
#         dlg.Destroy()
    ...
    def OnBuildScript(self, evt):
        ...
        if self.excel.GetLabelText() == "No selected":
            ...
            dlg.Destroy()
            return
#         if self.apk.GetLabelText() == "No selected":
...
#             apk2 = None
        if apk == None: 
            dlg = wx.MessageDialog(self, 'Please set source apk first',
                                'A Message Box',
                                wx.OK | wx.ICON_INFORMATION
                                #wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_INFORMATION
                                )
...
#         apk = (apk)
        ...
        self.buildDosScript(worksapce, excel, apk, rar, signer, self.buildNumber.GetLineText(0), self.password.GetLineText(0), keystore, self.apkName.GetLineText(0), self.aliasName.GetLineText(0))
...
#                 paramKeys.append(key)
        ...
        return
    def buildDosScript(self, workspace, excel, apk, rar, signer, code, key, store, appName, aliasName):
        print 'buildDosScript'
        signCmd = u'"%s" -sigalg SHA1withRSA -digestalg SHA1 -verbose -storepass %s -keypass %s -keystore "%s" -signedjar %s app.apk %s\r\n' % (signer, key, key, store, appName, aliasName)
        bat = codecs.open("\\".join([workspace, "build.bat"]), 'w', "gbk")
        s1 = "REM global settings\r\nmkdir .\\assets\r\nmkdir .\\out\r\ncopy \""
        s2 = "\" .\\app.apk\r\n"
        if apk != None:
            bat.write(s1+apk+s2)
            bat.write(u'"%s" d ".\\app.apk" META-INF\\*\r\n' % rar)
#         s3 = "copy \""
#         s4 = "\" .\\app2.apk\r\n"
#         if apk[1] != None :
...
#                     sourceApk = "app2.apk"
                if sourceApk == None :
                    continue
                chanel = sh.cell_value(rowx=i, colx=0)
                enName = sh.cell_value(rowx=i, colx=6)
                name = sh.cell_value(rowx=i, colx=1)
                name =  name.strip()
...
#                             bat.write(s)
                ...
                bat.write(u"copy " +appName+ u" \""+path+targetApk+u"\"\r\n")
...
class MyApp(wx.App):
    def OnInit(self):
        frame = MyFrame(None, "Apk Packer v1.0.0")
